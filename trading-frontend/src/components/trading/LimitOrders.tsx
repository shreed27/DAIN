...
import { useLimitOrderTriggers } from "@/lib/useWebSocket";
...
// Triggered Order Alert Component
function TriggeredOrderAlert({
  order,
  onExecute,
  onDismiss,
}: {
  ...
  return (
    <motion.div
      ...
      className="p-4 rounded-xl border-2 border-yellow-500/50 bg-yellow-500/10 mb-3"
    >
      <div className="flex items-center gap-3 mb-2">
        ...
        <div className="flex-1">
          <p className="font-semibold text-yellow-400">Limit Order Triggered!</p>
          <p className="text-xs text-muted-foreground">
            Target: {order.direction === 'above' ? '>' : '<'} {order.targetPrice.toFixed(6)} |
            Current: {order.currentPrice.toFixed(6)}
          </p>
        </div>
      </div>
      <div className="flex gap-2">
        <button
          ...
          className="flex-1 py-2 rounded-lg bg-yellow-500 hover:bg-yellow-400 text-black font-medium transition-colors flex items-center justify-center gap-2"
        >
        ...
        <button
          ...
          className="px-4 py-2 rounded-lg bg-white/10 hover:bg-white/20 text-muted-foreground font-medium transition-colors"
        >
          Dismiss
        </button>
      </div>
...
export function LimitOrders({ walletAddress, className, compact = false }: LimitOrdersProps) {
  ...
  // Listen for triggered orders via WebSocket
  const { triggeredOrders, clearTriggeredOrder } = useLimitOrderTriggers();

  // Filter triggered orders for this wallet
  const myTriggeredOrders = triggeredOrders.filter(
    (order) => walletAddress && order.walletAddress === walletAddress
  );

  const handleExecuteTriggered = useCallback(async (orderId: string) => {
    // TODO: Implement swap execution for triggered orders
    // This would call the swap-transaction endpoint and prompt wallet signing
    console.log("Execute triggered order:", orderId);
    clearTriggeredOrder(orderId);
    // Refresh orders list after execution
    fetchData(true);
  }, [clearTriggeredOrder]);

  const handleDismissTriggered = useCallback((orderId: string) => {
    clearTriggeredOrder(orderId);
  }, [clearTriggeredOrder]);
  ...
  return (
    <div className={cn("rounded-xl border border-white/5 bg-white/[0.02] p-4", className)}>
      ...
      {/* Triggered Order Alerts */}
      <AnimatePresence>
        {myTriggeredOrders.length > 0 && (
          <div className="mb-4">
            {myTriggeredOrders.map((order) => (
              <TriggeredOrderAlert
                key={order.id}
                order={order}
                onExecute={handleExecuteTriggered}
                onDismiss={handleDismissTriggered}
              />
            ))}
          </div>
        )}
